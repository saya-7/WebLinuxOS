...
const LineNumbers = memo(function LineNumbers({ lines, scrollTop }: { lines: number; scrollTop: number }) {
  ...
  return (
    <div
      ref={lineNumRef}
      className="notepad-line-numbers"
      style={{
        width: 40,
        background: '#181825',
        color: '#6c7086',
...
export default function Notepad() {
  ...
  }, [nameInput, fileId, renameFile])

  // 使用 useMemo 计算匹配数量，避免在 effect 中调用 setState
  ...
  }, [searchText, replaceText, content])
  ...
  return (
    <div className="app-container notepad-container" style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#1e1e2e', color: '#cdd6f4' }}>
      <div className="notepad-toolbar" style={{ display: 'flex', alignItems: 'center', padding: '4px 8px', borderBottom: '1px solid #313244', gap: 4, background: '#181825' }}>
        ...
        {editingName ? (
          ...
          <span 
            ...
            style={{ fontSize: 12, cursor: 'pointer', color: isModified ? '#f9e2af' : '#cdd6f4' }}
          >
            {fileName}{isModified ? ' *' : ''}
          </span>
        )}
      ...
      {(showSearch || showReplace) && (
        <div className="notepad-search-bar" style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 8px', background: '#181825', borderBottom: '1px solid #313244', flexWrap: 'wrap' }}>
          <input 
            value={searchText} 
            onChange={(e) => setSearchText(e.target.value)} 
            placeholder="查找..." 
            style={{ padding: '3px 8px', background: '#313244', border: '1px solid #45475a', borderRadius: 3, color: '#cdd6f4', fontSize: 12, width: 120, outline: 'none' }} 
          />
          <span style={{ fontSize: 11, color: '#6c7086' }}>{matchCount > 0 ? `${matchCount} 个匹配` : '无匹配'}</span>
          {showReplace && (
            <>
              <input 
                value={replaceText} 
